...
const TENANT_ESPERADO = 'mhzenergiasolar';
...
export function getGronerTenantInfo() {
  ...
  return {
    ...
    tenantOk: tenant === TENANT_ESPERADO,
    tenantHint:
      tenant && tenant !== TENANT_ESPERADO
        ? `Use GRONER_TENANT=${TENANT_ESPERADO} (atual tem ${tenant.length} caracteres)`
        : null,
  };
}
...
export async function validarConexaoGroner() {
  ...
  if (tenant !== TENANT_ESPERADO) {
    return {
      ok: false,
      code: 'GRONER_WRONG_TENANT',
      message: `Tenant incorreto. Configure GRONER_TENANT=${TENANT_ESPERADO}.`,
    };
  }
...
function erroGronerRelevante(err) {
  if (!err) return null;
  if (err.status === 401 || err.status === 403) {
    const e = new Error(
  ...
  if (err.status >= 500) {
    const e = new Error(
      `Erro no servidor Groner (${err.status}). Verifique GRONER_TENANT=${TENANT_ESPERADO} na Vercel.`,
    );
...
export async function diagnosticarBuscaGroner(query) {
  ...
  const resumo = (data, mapper) => {
    ...
    return {
      ...
      itemsType: data?.items == null ? 'null' : typeof data.items,
