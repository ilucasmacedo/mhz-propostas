...
function mapProjetoBusca(raw) {
  ...
  if (!p) return null;
  const lead = raw?.lead ?? raw?.Lead;
  return {
    ...p,
    leadId: p.leadId ?? lead?.id ?? lead?.Id ?? raw?.leadId ?? raw?.LeadId,
    leadNome: lead?.nome ?? lead?.Nome ?? null,
  };
}
...
/** Busca negócios (Projetos) — GET /api/Projeto/Pesquisar?query= */
async function buscarProjetosPorQuery(query) {
  ...
  try {
    let data = await gronerFetch(`/Projeto/Pesquisar?query=${encodeURIComponent(value)}`);
    let items = mapItems(data);
    if (items.length) return items;

    data = await gronerFetch(
      `/Projeto?query=${encodeURIComponent(value)}&pagina=1&tamanhoPagina=25`,
    );
