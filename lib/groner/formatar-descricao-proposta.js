import { getClienteNomeCurto, getClienteConfig } from '../cliente-config.js';
...
function montarTextoPlano(dados) {
  ...
  const nomeCurto = getClienteNomeCurto();
  const sistemaNome = getClienteConfig().produto?.sistemaNome || `${nomeCurto} Propostas`;

  const linhas = [
    `PROPOSTA COMERCIAL — ${nomeCurto}`,
    '',
  ...
  linhas.push('', `Gerado pelo sistema ${sistemaNome}`);
